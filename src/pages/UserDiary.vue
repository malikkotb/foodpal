  ...
  <div style="background-color: #f8f9fa">
    ...
    <div v-show="showDetailsFood" class="macroCard">
      ...
      <div class="text-center">
        ...
        <button @click="deleteEntry()" class="btn btn-danger btn-square mx-2">
          ...
          <div style="background-color: #f8f9fa">
            <div class="diary">
              ...
              <div class="meal-time mt-4 mb-2 mx-3 px-3 py-2">
                ...
                <p class="subtext">
                  {{
                    macrosDataLoaded ? macrosMeal("bf") : "Loading Macros bro"
                  }}
                </p>
              </div>
              <div
                ...
                @click="macrosFoodItem(item, 'breakfast')"
              ...
              <div
                ...
                :key="item"
                class="food-entry my-1 mx-3 px-3 py-2 d-flex align-items-center justify-content-between"
                @click="macrosFoodItem(item, 'lunch')"
                :style="foodEntryStyle"
              >
                <div>
                  <p>{{ item.foodName }}</p>
              ...
              <div
                ...
                :key="item"
                class="food-entry my-1 mx-3 px-3 py-2 d-flex align-items-center justify-content-between"
                @click="macrosFoodItem(item, 'dinner')"
                :style="foodEntryStyle"
              >
                <div>
                  <p>{{ item.foodName }}</p>
              ...
              <div
                ...
                :key="item"
                class="food-entry my-1 mx-3 px-3 py-2 d-flex align-items-center justify-content-between"
                @click="macrosFoodItem(item, 'snacks')"
                :style="foodEntryStyle"
              >
                <div>
                  <p>{{ item.foodName }}</p>
...
export default {
  data() {
    return {
      ...
      numServings: 1,
      foodData: [
        // default foodData List, can be ignored
  ...
  methods: {
    async saveEntry(foodEntry) {
      ...
      const newEntryRef = await push(entryRef, {
        ...foodEntry,
        timeAdded: currentDateTime,
      });

      // store the key of each individual food item inside the object in firebase, for later use like locate/delete
      const newPostKey = newEntryRef.key;
      const updateRef = ref(
        db,
        "users/" +
          "userId/" +
      ...
      await update(updateRef, { postKey: newPostKey });

      // refresh data
      this.firebaseData = this.getFoodFromFirebase();
    },
    async deleteEntry() {
      console.log("selected foodEntryId: " + this.foodEntryId);
    ...
    updateActiveSlide(operation) {
      ...
      this.firebaseData = this.getFoodFromFirebase(); // this loads data for a new Date
    },
    getBreakfast() {
    ...
    getLunch() {
      if (this.isUpdated && this.firebaseData) {
        if (this.dateList && this.activeSlide !== null) {
          let lunchData = [];
          for (let key in this.firebaseData.lunch) {
            lunchData.push(this.firebaseData.lunch[key]);
          }
          this.macrosDataLoaded = true;
          return lunchData;
        }
    ...
    getDinner() {
      if (this.isUpdated && this.firebaseData) {
        if (this.dateList && this.activeSlide !== null) {
          let dinnerData = [];
          for (let key in this.firebaseData.dinner) {
            dinnerData.push(this.firebaseData.dinner[key]);
          }
          this.macrosDataLoaded = true;
          return dinnerData;
        }
    ...
    getSnacks() {
      if (this.isUpdated && this.firebaseData) {
        if (this.dateList && this.activeSlide !== null) {
          let snackData = [];
          for (let key in this.firebaseData.snacks) {
            snackData.push(this.firebaseData.snacks[key]);
          }
          this.macrosDataLoaded = true;
          return snackData;
        }
    ...
    pushItem(newEntry) {
      ...
      const editedEntry = {
        ...newEntry,
        foodName: this.capitalizeLetter(newEntry.foodName),
        calories: Math.floor(newEntry.calories * this.numServings),
        macronutrients: {
    ...
    macrosMeal(category) {
      ...
      switch (category) {
        case "bf":
          foodEntries = this.getBreakfast();
          break;
        case "lunch":
          foodEntries = this.getLunch();
          break;
        case "dinner":
          foodEntries = this.getDinner();
          break;
        case "snacks":
          foodEntries = this.getSnacks();
          break;
        default:
          break;
      }
    ...
    macrosFoodItem(item, category) {
      this.selectedFood = item;
      this.foodEntryId = item.postKey;
    ...
    async fetchFoodData(query) {
      ...
      await fetch(instantUrl, {
        ...
        .then((data) => {
          ...
          // TODO: change to i < 10 -> to get top 10 results and not only top 3
          for (let i = 0; i < 3; i++) {
            const foodItem = {
